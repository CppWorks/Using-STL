...
{ // n_x is plot width in characters, so it's the number of characters along the x axis
  ...
  if (max_x > 0.0 && min_x <= 0.0) {
    ...
    std::for_each(std::begin(rows), std::end(rows), [x_axis](string& row) {
  ...
  for (auto& row : rows) {
    ...
    std::copy_if(
      begin_iter, end_iter, std::back_inserter(row_pts),
...
{ // n_pts is number of data points for the curve
  ...
  std::generate(std::begin(x_values), std::end(x_values), [&value, &step]() {
    double v{ value };
    value += step;
    return v;
  });
