...
valarray<double> get_data(size_t n)
{
  ...
  std::cout << "Enter " << n + 1 << " values for each of " << n << " equations.\n"
            << "(i.e. including coefficients that are zero and the rhs:\n";
  for (auto& coeff : equations)
    std::cin >> coeff;
  return equations;
}

// Selects the best pivot in row n (rows indexed from 0)
void set_pivot(const valarray<double>& equations, std::vector<slice>& row_slices,
               size_t n)
{
  ...
  {
    if (column[max_index] < column[i])
      max_index = i;
  }

  if (max_index > 0) { // Pivot is not the 1st in column - so swap rows to make it so
                                   ...
                                   const std::vector<slice>& row_slices)
{
  ...
  // Multiply the last row by the second to last element in each row and subtract it from
  // the row. Repeat for all the other rows
