...
void list_DOB_Job(const People& people)
{
  ...
  for (auto iter = std::begin(people); iter != std::end(people); ++iter) {
    ...
    std::cout << std::setw(20) << std::left
              << (iter->first.first + " " + iter->first.second) << "DOB: " << std::right
              << std::setw(2) << std::get<0>(dob) << "-" << std::setw(2)
              << std::setfill('0') << std::get<1>(dob) << "-" << std::setw(4)
              << std::get<2>(dob) << std::setfill(' ') << "  Occupation: " << occupation
              << std::endl;
  }
}
...
void list_sorted_people(const People& people, Compare comp)
{
  ...
  std::sort(std::begin(folks), std::end(folks),
            ptr_comp); // Sort the pointers to elements
