...
int main()
{
  ...
  std::array<Die, n_dice> dice // The dice
    { Die{ 3, 3, 3, 3, 3, 3 }, Die{ 0, 0, 4, 4, 4, 4 }, Die{ 1, 1, 1, 5, 5, 5 },
      Die{ 2, 2, 2, 2, 6, 6 } };
  ...
  while (true) {
    ...
    std::array<std::pair<size_t, size_t>, n_throws>
      goes; // Results of goes - pair<me_value, you_value>
    std::generate(std::begin(goes), std::end(goes), // Make the throws
                  [&dice, me, you] {
                    return std::make_pair(dice[me].throw_it(), dice[you].throw_it());
                  });
    ...
    std::for_each(
      ...
      [](const std::pair<size_t, size_t>& pr) { std::cout << std::setw(3) << pr.first; });
    auto my_wins = std::count_if(
      std::begin(goes), std::end(goes),
      [](const std::pair<size_t, size_t>& pr) { return pr.first > pr.second; });
    std::cout << " My wins:   " << std::setw(2) << std::right << my_wins << "   I "
              << ((my_wins > n_throws / 2) ? "win!!" : "lose {:-(") << std::endl;
