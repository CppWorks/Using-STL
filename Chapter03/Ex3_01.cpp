...
inline size_t precedence(const char op)
{
  ...
  if (op == '^')
    return 3;
  throw std::runtime_error{ string{ "invalid operator in precedence() function: " }
                            + op };
}
...
double execute(std::stack<char>& ops, std::stack<double>& operands)
{
  ...
  case '^':
    ...
    break;
  default:
    throw std::runtime_error{ string{ "invalid operator in execute() function: " }
                              + ops.top() };
  }
...
int main()
{
  ...
  std::cout << "An arithmetic expression can include the operators +, -, *, /, and ^ for "
               "exponentiation."
            << std::endl;
  try {
    while (true) {
