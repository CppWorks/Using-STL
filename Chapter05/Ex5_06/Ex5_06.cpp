...
void make_friends(Names& names)
{
  ...
  std::vector<string> first_names{ "John", "John", "John", "Joan",
                                   "Joan", "Jim",  "Jim",  "Jean" };
...
void list_buckets(const Names& names)
{
  for (size_t n_bucket{}; n_bucket < names.bucket_count(); ++n_bucket) {
    ...
    std::copy(names.begin(n_bucket), names.end(n_bucket),
...
int main()
{
  ...
  while (true) {
    auto iter = std::find_if(std::begin(pals), std::end(pals), [](const Name& name) {
